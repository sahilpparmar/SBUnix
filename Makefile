...
submit: clean
	tar -czvf $(USER).tgz --exclude=.empty --exclude=.*.sw? --exclude=*~ LICENSE README qstart.sh Makefile linker.script sys bin crt libc ld newfs include $(ROOTFS) $(USER).img test.img 
