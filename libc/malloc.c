...
{
    char *add;
    int no_of_pages = 0, sz;    
    MCB_P p_mcb;
    
    sz = sizeof(MCB);
    no_of_pages = (aligned_size + sz) /(PAGESIZE + 1) + 1;
    add         = (char *)__syscall1(BRK, no_of_pages);    
    //add       = virt_alloc_pages(no_of_pages); 
    ...
    heap_end = (void *)add + PAGESIZE * no_of_pages; 
...
void *malloc(int elem_size)
{
    ...
    if(heap_end == NULL) /*no heap has been assigned yet*/
    {
